...
while True:
    ...
    if hands and not handFlip and not buttonPressed:
        ...
        if finger == [0,1,1,0,0] or finger == [1,1,1,0,0]:
            cv2.circle(currentSlide,indexFinger,5,(0,0,255),indexThickness,cv2.FILLED)
            annotStart = False
    ...
    for i in range(len(annotations)):
        for j in range(len(annotations[i])):            
            if j!=0:
                cv2.line(currentSlide,annotations[i][j-1],annotations[i][j],(0,0,0),5)
    ...
    cv2.putText(img, "FPS:" + str(fps), (10, 30), cv2.FONT_HERSHEY_SIMPLEX,1.0, (0, 0, 0), 4, cv2.LINE_AA)
    cv2.putText(img, "FPS:" + str(fps), (10, 30), cv2.FONT_HERSHEY_SIMPLEX,1.0, (255, 255, 255), 2, cv2.LINE_AA)
